...
def dwf_search_for_devices():
    ...
    if result == False:
        ...
        windows_api.windows_message("Device found.", "Found " + num_of_devices_connected + " devices. Opening first", 0)
...
def dwf_generate_custom_waveform(dwf_ao, channel, frequency, amplitude, rgdSamples_custom_values):
    ...
    dwf_ao.nodeOffsetSet(channel, dwf_ao.NODE.CARRIER, rgdSamples_custom_values)
...
def exit_program(num_of_case):
    if num_of_case == int(exit_cases.NO_DEVICES_FOUND):
        windows_api.windows_message("Error", "No connected device detected.", 0)
